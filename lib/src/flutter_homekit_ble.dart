...
class FlutterHomeKit {
  ...
  static final MethodChannel _methodChannel = const MethodChannel('flutter_homekit6in1/methods');
  ...
  static Future<void> turnOn({int timeout = 60}) async {
    ...
    if (changed) {
      ...
      if (response.userAccepted == false) {
        throw FlutterHomeKitException(ErrorPlatform.fbp, "turnOn", FbpErrorCode.userRejected.index, "user rejected");
      }
  ...
  static Future<void> startScan({
    ...
    try {
      ...
      Stream<BmScanResponse> responseStream = FlutterHomeKit._methodStream.stream
      ...
      _scanSubscription = outputStream.listen((BmScanResponse? response) {
        if (response == null) {
          ...
          if (response.success == false) {
            var e = FlutterHomeKitException(_nativeError, "scan", response.errorCode, response.errorString);
  ...
  static void cancelWhenScanComplete(StreamSubscription subscription) {
    FlutterHomeKit._scanSubscriptions.add(subscription);
  }
  ...
  static Future<dynamic> _initFlutterHomeKit() async {
    if (_initialized) {
      return;
    }
...
class FlutterHomeKitException implements Exception {
  ...
  FlutterHomeKitException(this.platform, this.function, this.code, this.description);

  @override
  String toString() {
    String sPlatform = platform.toString().split('.').last;
    return 'FlutterHomeKitException | $function | $sPlatform-code: $code | $description';
  }
