...
class BluetoothDescriptor {
  ...
  List<int> get lastValue {
    ...
    return FlutterHomeKit._lastDescs[remoteId]?[key] ?? [];
  }
  ...
  Future<List<int>> read({int timeout = 15}) async {
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('readDescriptor', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "readDescriptor", response.errorCode, response.errorString);
      }
  ...
  Future<void> write(List<int> value, {int timeout = 15}) async {
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('writeDescriptor', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "writeDescriptor", response.errorCode, response.errorString);
      }
    } finally {
      mtx.give();
    }
