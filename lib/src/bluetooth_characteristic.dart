...
class BluetoothCharacteristic {
  ...
  List<int> get lastValue {
    ...
    return FlutterHomeKit._lastChrs[remoteId]?[key] ?? [];
  }
  ...
  Stream<List<int>> get lastValueStream => FlutterHomeKit._methodStream.stream
  ...
  ///   - anytime `read()` is called
  ///   - anytime a notification arrives (if subscribed)
  Stream<List<int>> get onValueReceived => FlutterHomeKit._methodStream.stream
  ...
  Future<List<int>> read({int timeout = 15}) async {
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('readCharacteristic', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "readCharacteristic", response.errorCode, response.errorString);
      }
  ...
  Future<void> write(List<int> value,
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('writeCharacteristic', request.toMap());
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "writeCharacteristic", response.errorCode, response.errorString);
      }
  ...
  Future<bool> setNotifyValue(bool notify, {int timeout = 15, bool forceIndications = false}) async {
    ...
    try {
      ...
      Stream<BmDescriptorData> responseStream = FlutterHomeKit._methodStream.stream
      ...
      // invoke
      bool hasCCCD = await FlutterHomeKit._invokeMethod('setNotifyValue', request.toMap());
      ...
      if (hasCCCD) {
        ...
        // failed?
        if (!response.success) {
          throw FlutterHomeKitException(_nativeError, "setNotifyValue", response.errorCode, response.errorString);
        }
      }
    } finally {
      mtx.give();
  ...
  BmBluetoothCharacteristic? get _bmchr {
    if (FlutterHomeKit._knownServices[remoteId] != null) {
      for (var s in FlutterHomeKit._knownServices[remoteId]!.services) {
