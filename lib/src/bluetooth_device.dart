...
class BluetoothDevice {
  ...
  String get platformName => FlutterHomeKit._platformNames[remoteId] ?? "";

  /// Advertised Named
  ///  - this is the name advertised by the device during scanning
  ///  - it is only available after you scan with FlutterHomeKit
  ///  - it is cleared when the app restarts.
  ///  - not all devices advertise a name
  String get advName => FlutterHomeKit._advNames[remoteId] ?? "";
  ...
  List<BluetoothService> get servicesList {
    BmDiscoverServicesResult? result = FlutterHomeKit._knownServices[remoteId];
    if (result == null) {
      return [];
    } else {
  ...
  void cancelWhenDisconnected(StreamSubscription subscription, {bool next = false, bool delayed = false}) {
    if (isConnected == false && next == false) {
      ...
      FlutterHomeKit._delayedSubscriptions[remoteId] ??= [];
      FlutterHomeKit._delayedSubscriptions[remoteId]!.add(subscription);
    } else {
      FlutterHomeKit._deviceSubscriptions[remoteId] ??= [];
      FlutterHomeKit._deviceSubscriptions[remoteId]!.add(subscription);      
    }
  }

  /// Returns true if autoConnect is currently enabled for this device
  bool get isAutoConnectEnabled {
    return FlutterHomeKit._autoConnect.contains(remoteId);
  }

  /// Returns true if this device is currently connected to your app
  bool get isConnected {
    if (FlutterHomeKit._connectionStates[remoteId] == null) {
      return false;
    } else {
      var state = FlutterHomeKit._connectionStates[remoteId]!.connectionState;
  ...
  Future<void> connect({
    ...
    try {
      ...
      if (autoConnect) {
        FlutterHomeKit._autoConnect.add(remoteId);
      }
      ...
      var responseStream = FlutterHomeKit._methodStream.stream
      ...
      // invoke
      bool changed = await FlutterHomeKit._invokeMethod('connect', request.toMap());
      ...
      if (changed && !autoConnect) {
        BmConnectionStateResponse response = await futureState
            ...
            .catchError((e) async {
          if (e is FlutterHomeKitException && e.code == FbpErrorCode.timeout.index) {
            await FlutterHomeKit._invokeMethod('disconnect', remoteId.str); // cancel connection attempt
          }
          throw e;
        });
  ...
  Future<void> disconnect({int timeout = 35, bool queue = true}) async {
    ...
    try {
      ...
      FlutterHomeKit._autoConnect.remove(remoteId);
      ...
      // invoke
      bool changed = await FlutterHomeKit._invokeMethod('disconnect', remoteId.str);
  ...
  Future<List<BluetoothService>> discoverServices({bool subscribeToServicesChanged = true, int timeout = 15}) async {
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('discoverServices', remoteId.str);
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "discoverServices", response.errorCode, response.errorString);
      }
  ...
  DisconnectReason? get disconnectReason {
    ...
    int? code = FlutterHomeKit._connectionStates[remoteId]!.disconnectReasonCode;
    String? description = FlutterHomeKit._connectionStates[remoteId]!.disconnectReasonString;
  ...
  Stream<BluetoothConnectionState> get connectionState {
    ...
    if (FlutterHomeKit._connectionStates[remoteId] != null) {
      initialValue = _bmToConnectionState(FlutterHomeKit._connectionStates[remoteId]!.connectionState);
    }
    return FlutterHomeKit._methodStream.stream
        .where((m) => m.method == "OnConnectionStateChanged")
        .map((m) => m.arguments)
        .map((args) => BmConnectionStateResponse.fromMap(args))
        .where((p) => p.remoteId == remoteId)
  ...
  int get mtuNow {
    ...
    return FlutterHomeKit._mtuValues[remoteId]?.mtu ?? 23;
  }
  ...
  Stream<int> get mtu {
    // get initial value from our cache
    int initialValue = FlutterHomeKit._mtuValues[remoteId]?.mtu ?? 23;
    return FlutterHomeKit._methodStream.stream
  ...
  Future<int> readRssi({int timeout = 15}) async {
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('readRssi', remoteId.str);
      ...
      // failed?
      if (!response.success) {
        throw FlutterHomeKitException(_nativeError, "readRssi", response.errorCode, response.errorString);
      }
      rssi = response.rssi;
    } finally {
      mtx.give();
  ...
  Future<int> requestMtu(int desiredMtu, {double predelay = 0.35, int timeout = 15}) async {
    ...
    if (Platform.isAndroid == false) {
      throw FlutterHomeKitException(ErrorPlatform.fbp, "requestMtu", FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    try {
      ...
      // invoke
      await FlutterHomeKit._invokeMethod('requestMtu', request.toMap());
  ...
  Future<void> requestConnectionPriority({required ConnectionPriority connectionPriorityRequest}) async {
    ...
    // check connected
    if (isDisconnected) {
      throw FlutterHomeKitException(ErrorPlatform.fbp, "requestConnectionPriority",
    ...
    // invoke
    await FlutterHomeKit._invokeMethod('requestConnectionPriority', request.toMap());
  }
  ...
  Future<void> setPreferredPhy({
    ...
    // invoke
    await FlutterHomeKit._invokeMethod('setPreferredPhy', request.toMap());
  }
  ...
  Future<void> createBond({int timeout = 90}) async {
    // check android
    if (Platform.isAndroid == false) {
      throw FlutterHomeKitException(ErrorPlatform.fbp, "createBond", FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    try {
      ...
      // invoke
      bool changed = await FlutterHomeKit._invokeMethod('createBond', remoteId.str);
      ...
      if (changed) {
        ...
        if (bs.bondState != BmBondStateEnum.bonded) {
          throw FlutterHomeKitException(ErrorPlatform.fbp, "createBond", FbpErrorCode.createBondFailed.hashCode,
  ...
  Future<void> removeBond({int timeout = 30}) async {
    // check android
    if (Platform.isAndroid == false) {
      throw FlutterHomeKitException(ErrorPlatform.fbp, "removeBond", FbpErrorCode.androidOnly.index, "android-only");
    }
    ...
    try {
      ...
      // invoke
      bool changed = await FlutterHomeKit._invokeMethod('removeBond', remoteId.str);
      ...
      if (changed) {
        ...
        if (bs.bondState != BmBondStateEnum.none) {
          throw FlutterHomeKitException(ErrorPlatform.fbp, "createBond", FbpErrorCode.removeBondFailed.hashCode,
  ...
  Future<void> clearGattCache() async {
    ...
    // invoke
    await FlutterHomeKit._invokeMethod('clearGattCache', remoteId.str);
  }
  ...
  Stream<BluetoothBondState> get bondState async* {
    // check android
    if (Platform.isAndroid == false) {
      throw FlutterHomeKitException(ErrorPlatform.fbp, "bondState", FbpErrorCode.androidOnly.index, "android-only");
    }

    // get current state if needed
    if (FlutterHomeKit._bondStates[remoteId] == null) {
      var val = await FlutterHomeKit._methodChannel
      ...
      if (FlutterHomeKit._bondStates[remoteId] == null) {
        FlutterHomeKit._bondStates[remoteId] = val;
      }
    }

    yield* FlutterHomeKit._methodStream.stream
        .where((m) => m.method == "OnBondStateChanged")
        .map((m) => m.arguments)
        .map((args) => BmBondStateResponse.fromMap(args))
        .where((p) => p.remoteId == remoteId)
        .map((p) => _bmToBondState(p.bondState))
        .newStreamWithInitialValue(_bmToBondState(FlutterHomeKit._bondStates[remoteId]!.bondState));
  }

  /// Get the previous bondState of the device (Android Only)
  BluetoothBondState? get prevBondState {
    var b = FlutterHomeKit._bondStates[remoteId]?.prevState;
  ...
  String toString() {
    return 'BluetoothDevice{'
        ...
        'services: ${FlutterHomeKit._knownServices[remoteId]}'
        '}';
  }
